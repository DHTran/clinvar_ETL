...
class ClinVar_Parse_deprecated:
    ...
    blocklist_pmids contains list of pmids that are not case or
    functional studies (manually curated)
    """
    ...
    def parse_clinvar_records(self, ids_records):
        ...
        for id_record in ids_records:
            ...
            # checks for 'VariationType="copy number',
    ...
    def get_target_indices(self, record):
        ...
        for key in target_indices.keys():
            # VCF_CONDITIONS is tuple with two potential matches when
            # no VCF data is present, must match both targets in line
    ...
    def get_submitters_and_submitted_data(self, target_indices, record):
        ...
        def get_classification_and_comments(index, record):
            ...
            ClinVar records.  Counts number of different classifications
            in records from a gene.

            the classification and comments positions may vary after
            the line containing 'Description>' and 'Comment>'
            respectively
        ...
        for gene, data in self.load_gene_json(
            ...
            if file_.is_file():
                if self.overwrite:
                    ...
                    continue
            sorted_df.to_csv(
                csvs_path/filename, encoding='utf-8', index=False)
